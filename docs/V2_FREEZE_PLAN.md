...
| Cloud-bridge push | **FROZEN** | `lib/cloudBridge.ts:92` `pushSnapshot()`→`pushBatch()`; DLQ admin routes complete; `test-cloud-bridge-replay-determinism.mjs` pins local replay state-equivalence; /v2 canonical envelope + per-table schema_version pinned by test-cloud-bridge-envelope-golden.mjs + coverage + test-cloud-bridge-canonical.mjs (PROTECTED_CONTRACTS §11.4) | |
...
| Subsystem | Status | Evidence | Note |
|-----------|--------|----------|------|
| KDS server protocol | **FROZEN** | `lib/kds.ts:100` `BumpResponse {id, bumped_at}`; bump route + tables; `test-kds-bump-route.mjs`; `Lariat-KDS` PR #2 merged at `84adff3` (head `c694206`) with Swift parser + doc sentinel tests | Roadmap 1.3 and 1.12 done |
