...
3.  **Runtime Read Model**:
    - **JSON cache** (`data/cache/*.json`, 14 files) — templates: recipes, stations, staff, line_checks, setups, menu, allergen_matrix, food_safety, vendor_summary, labor_summary, closings, weekly_prep, order_guide, catering_menu. Read by `lib/data.ts` (mtime-aware in-memory cache). Parse-failure semantics (GH #252): a corrupted JSON file does NOT clobber the in-memory cache. If a prior-good entry exists `lib/data` keeps serving it and marks the file in `getCacheHealth().degraded` so the freshness banner / `/api/data/health` can surface the broken state — yesterday's allergen tags stay visible until the file is repaired, instead of silently flipping to "no allergens tagged."
