...
3.  **Runtime Read Model**:
    ...
    - **SQLite** (`data/lariat.db`) — 40+ tables grouped by family:
        ...
        - **Costing (T1–T9):** `vendor_prices`, `recipe_costs`, `bom_lines`, `ingredient_maps`, `ingredient_masters`, `ingredient_densities`, `ingredient_yields`, `ingredient_unit_weights`, `vendor_catch_weights`, `pack_size_changes`, `ingest_runs`, `order_guide_items`
...
`costingBenchmarks.mjs` (T9 variance, T7 merged cost, B1/B2), `unitConvert.mjs` (byte-parity with `scripts/lib/units.py`), `ingredientKey.ts` (byte-parity with `scripts/lib/ingredient_key.py`), `dishCostBridge.ts` (dish→recipe→ingredient→vendor link_state), `menuEngineering.ts` (star/puzzle/plowhorse/dog quadrant), `bomVendorProposals.ts`, `bomPlanActionItems.mjs`, `subRecipeGraph.ts` (cascaded 86), `recipeCalculator.ts` (spawns `scripts/bom_expand_cli.py`), `dishComponents.ts`.
