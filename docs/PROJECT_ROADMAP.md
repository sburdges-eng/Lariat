...
| 2.6 | L  | **Partially closed (2026-06-11, PR #320):** Long-press push-to-talk shipped in the LaRi composer via the Web Speech API (pointer + keyboard hold-to-talk with stop-on-submit/blur/escape/tab-hide and error recovery; `app/kitchen-assistant/KitchenAssistantClient.jsx`). Residual scope: swap the recognition backend to local Whisper-tiny so dictation works offline / without Apple's speech service — keep deferred per the freeze note below (no cloud Whisper). |
...
| 2.21 | M  | **Instrumented / hardware pending:** Cook-tier iPad profiling now has a local Playwright harness for station pass, KDS send, and 86 add tap-to-feedback flows (`npm run profile:ipad`, pinned by `npm run test:ipad-profile`). 2026-06-11: harness extended with `--route-prefix /v2` to drive the v2 tree (preview cookie auto-set); software-only chromium run confirms all three flows execute on `/v2` within threshold — see `docs/audit/2026-06-11-v2-stage0-readiness-evidence.md`. The row remains open until a real low-power iPad gen 7 run proves every flow has p95 <= 100ms and the JSON report is attached as release evidence. |
