...
| 1.3 | M      | **Closed:** `/api/kds/tickets/[id]/bump` response fields are pinned against `Lariat-KDS/docs/lariat-kds-protocol.md`. | `tests/js/test-kds-bump-route.mjs` parses the protocol doc's `Response schema (200)` JSON block and asserts the route response keys match exactly; CI checks out `Lariat-KDS` and runs `npm run test:kds-bump` so doc/route drift fails the build. |
