...
import { normalizeIngredientKey, deriveMasterId } from './ingredientKey.ts';
import { convertQty, convertPackSizeToLineUnit, normalizeUnit, effectivePackPrice } from './unitConvert.mjs';

/**
 * Row shapes read via raw SQL from an already-open better-sqlite3 handle.
 * better-sqlite3's `.all()`/`.get()` return `unknown` — these typedefs pin
 * down the columns each query in this file actually selects (mirrors the
 * CREATE TABLE / ALTER TABLE statements in lib/db.ts).
 *
 * @typedef {{ recipe_id: string, recipe_name: string | null, cost_per_yield_unit: number | null, yield: number | null }} RecipeCostRow
 * @typedef {{ recipe_id: string, ingredient: string | null, master_id: string | null, qty: number | null, unit: string | null, pack_price: number | null, pack_size: number | null, yield_pct: number | null, loss_factor: number | null }} BomLineRow
 * @typedef {{ ingredient: string | null, master_id: string | null, vendor: string | null, pack_price: number | null, pack_size: number | null, pack_unit: string | null, unit_price: number | null, id: number }} VendorPriceRow
 * @typedef {{ ingredient_key: string, g_per_ml: number }} DensityRow
 * @typedef {{ ingredient_key: string, unit: string, g_per_unit: number }} UnitWeightRow
 * @typedef {{ master_id: string, preferred_vendor: string | null }} MasterPreferredVendorRow
 * @typedef {{ recipe_id: string, yield: number | null, yield_unit: string | null, batch_cost: number | null }} SubRecipeCostRow
 * @typedef {{ recipe_id: string, recipe_name: string | null, theoretical: number, actual: number | null, variance_pct: number | null, total_lines: number, unmatched_lines: number, excluded: boolean, exclusion_reason: string | null }} CostVarianceRow
 */

/**
 * Type-predicate guard: a {@link CostVarianceRow} that is not excluded and
 * carries a numeric `variance_pct` — the subset the B1 aggregate (max/mean/
 * over5/over2 counters) is computed over.
 * @param {CostVarianceRow} p
 * @returns {p is CostVarianceRow & { variance_pct: number }}
 */
function hasNumericVariance(p) {
  return !p.excluded && p.variance_pct != null;
}

/**
 * T3-identical adjustment factor: 1 / (yield × (1 − loss)) with null defaults.
 * @param {number | null | undefined} yieldPct
 * @param {number | null | undefined} lossFactor
 * @returns {number | null}
 */
...
/**
 * Type-predicate guard used to filter `vendor_prices`-shaped rows down to
 * the subset with usable (non-null, positive, finite) pack_price/pack_size
 * before `resolveMergedCost` does any arithmetic on them.
 * @param {{vendor: string | null, pack_price: number | null, pack_size: number | null, pack_unit?: string | null, unit_price?: number | null} | null | undefined} r
 * @returns {r is {vendor: string | null, pack_price: number | null, pack_size: number, pack_unit?: string | null, unit_price?: number | null}}
 */
function isUsableVendorPriceRow(r) {
  if (r == null || r.pack_size == null || !(r.pack_size > 0) || !Number.isFinite(r.pack_size)) {
    return false;
  }
  const pp = effectivePackPrice(r);
  return pp != null && pp > 0 && Number.isFinite(pp);
}
 ...
 *
 * `pack_unit` rides along so unit-aware pricing can convert the pack into
 * the BOM line's unit: the preferred-vendor row's unit verbatim, or — for
 * the mean — the shared canonical unit when every contributing row agrees,
 * else null (callers fall back to the identity assumption).
 *
 * @param {{vendor: string | null, pack_price: number | null, pack_size: number | null, pack_unit?: string | null}[]} rows
 * @param {string | null} preferredVendor
 * @returns {{pack_price: number, pack_size: number, pack_unit: string | null, source: 'preferred_vendor' | 'mean'} | null}
 */
export function resolveMergedCost(rows, preferredVendor) {
  ...
  const usable = (rows ?? []).filter(isUsableVendorPriceRow);
  if (usable.length === 0) return null;

  if (preferredVendor) {
    const hit = usable.find((r) => r.vendor === preferredVendor);
    if (hit) {
      const packPrice = effectivePackPrice(hit);
  ...
  const meanUnitCost = latest.reduce((s, r) => {
    const pp = effectivePackPrice(r);
    return pp == null ? s : s + pp / r.pack_size;
  }, 0) / n;
  // The mean's unit is only meaningful when every contributing row prices
  // in the same canonical unit; otherwise leave it null (identity fallback).
  const canon = normalizeUnit(first.pack_unit);
  const uniformUnit =
    canon && latest.every((r) => normalizeUnit(r.pack_unit) === canon)
      ? (first.pack_unit ?? null)
      : null;
  return { pack_price: meanUnitCost, pack_size: 1, pack_unit: uniformUnit, source: 'mean' };
}
...
export function computeCostVariance(db, locationId, opts = {}) {
  ...
  const recipes = /** @type {RecipeCostRow[]} */ (
    db
      .prepare(
  ...
  const bomRows = /** @type {BomLineRow[]} */ (
    db
      .prepare(
        `SELECT recipe_id, ingredient, master_id, qty, unit, pack_price, pack_size,
  ...
  const vpRows = /** @type {VendorPriceRow[]} */ (
    db
      .prepare(
        `SELECT ingredient, master_id, vendor, pack_price, pack_size, pack_unit, unit_price, id
           FROM vendor_prices
          WHERE location_id = ?
  ...
  /** @type {Map<string, VendorPriceRow>} */
  ...
  // Unit-conversion seeds for the leaf path — same tables the ingest T4
  // pass reads. Loaded once per call; both are small curated seed tables.
  /** @type {Map<string, number>} */
  const densityByKey = new Map();
  const densityRows = /** @type {DensityRow[]} */ (
    db.prepare(`SELECT ingredient_key, g_per_ml FROM ingredient_densities`).all()
  );
  ...
  const unitWeightRows = /** @type {UnitWeightRow[]} */ (
    db.prepare(`SELECT ingredient_key, unit, g_per_unit FROM ingredient_unit_weights`).all()
  );
  for (const row of unitWeightRows) {
    const canon = normalizeUnit(row.unit);
    if (!canon || row.g_per_unit == null) continue;
    let m = unitWeightByKey.get(row.ingredient_key);
    if (!m) { m = new Map(); unitWeightByKey.set(row.ingredient_key, m); }
    m.set(canon, row.g_per_unit);
  }
  ...
  if (vpByMaster.size > 0) {
    const masterRows = /** @type {MasterPreferredVendorRow[]} */ (
      db.prepare(`SELECT master_id, preferred_vendor FROM ingredient_masters`).all()
    );
  ...
  // T10 sub-recipe fallback: build a recipe_id → recipe_costs row map so the
  // per-line loop can resolve sub-recipe ingredients to their stored
  // batch_cost / yield without a per-line DB query.
  /** @type {Map<string, SubRecipeCostRow>} */
  const subRecipeById = new Map();
  const subRecipeRows = /** @type {SubRecipeCostRow[]} */ (
    db
      .prepare(
  ...
  for (const r of subRecipeRows) {
    subRecipeById.set(r.recipe_id, r);
  }

  /** @type {Map<string, BomLineRow[]>} */
  const bomByRecipe = new Map();
  for (const r of bomRows) {
    let bucket = bomByRecipe.get(r.recipe_id);
    if (!bucket) { bucket = []; bomByRecipe.set(r.recipe_id, bucket); }
    bucket.push(r);
  }
  ...
  for (const r of recipes) {
    ...
    if (theoretical == null || !(theoretical > 0)) continue;
    // Defensive re-check: the SQL WHERE clause above already guarantees
    // `yield IS NOT NULL AND yield > 0`, so this never actually fires —
    // it exists to pin `r.yield` to `number` for the division at the
    // bottom of this loop.
    if (r.yield == null || !(r.yield > 0)) continue;
    const recipeYield = r.yield;
    ...
    for (const line of lines) {
      ...
      let packUnit = null;
      let matched = false;
      const masterRows = line.master_id ? vpByMaster.get(line.master_id) : undefined;
      if (line.master_id && masterRows) {
      ...
      if (!matched) {
        ...
        const eff = vp ? effectivePackPrice(vp) : null;
        if (vp && eff != null && vp.pack_size != null && vp.pack_size > 0) {
          packPrice = eff;
          packSize = vp.pack_size;
          packUnit = vp.pack_unit ?? null;
          matched = true;
        }
      }

      if (!matched) {
        // T10 sub-recipe fallback: if the line's normalized ingredient
        // resolves to an existing recipe_id, use that recipe's stored
        // batch_cost / yield as the unit cost (in child.yield_unit),
        // convert the line's qty from line.unit → child.yield_unit via
        // convertQty, and contribute to actualBatch. convertQty returns
        // null on cross-dimensional conversions without a density, which
        // we treat as still-unmatched so the D6 ratio gate fires.
      ...
      // T4 parity: convert pack_size from the vendor's pack_unit into the
      // line's unit before the ratio. Pre-fix this multiplied a cup qty
      // against a per-lb pack price as if the units matched, fabricating
      // an "actual" that tracked the workbook instead of reality. A line
      // whose conversion cannot complete (cross-dim without density, count
      // without unit_weight, unknown unit) counts as unmatched so the D6
      // ratio gate fires rather than reporting a made-up number.
      const lineKey = normalizeIngredientKey(line.ingredient ?? '');
      const { value: packSizeInLineUnit } = convertPackSizeToLineUnit(
        packSize,
        packUnit,
        line.unit,
        lineKey ? densityByKey.get(lineKey) : undefined,
        lineKey ? unitWeightByKey.get(lineKey) : undefined,
      );
      if (packSizeInLineUnit == null) {
        unmatchedLines += 1;
        continue;
      }
      actualBatch += (qty * packPrice / packSizeInLineUnit) * adj;
      contributed += 1;
    }
    ...
    const actual = actualBatch / recipeYield;
  ...
  const included = perRecipe.filter(hasNumericVariance);
...
/**
 * @typedef {{ id: number, recipe_id: string, ingredient: string | null, pack_price: number | null, pack_size: number | null, yield_pct: number | null, map_status: string | null, recipe_name: string | null }} UnmappedBomRow
 * @typedef {{ id: number, ingredient: string | null, vendor: string | null, sku: string | null }} PackChangedRow
 */

const GOOD_STATUSES = new Set(['confirmed', 'mapped', 'auto_mapped', 'no_cost_utility']);
 ...
 * signal and shouldn't inflate the BOM coverage denominator.
 * `unmapped_count` counts only BOM-unmapped lines so `unmapped_pct`
 * stays a pure BOM coverage ratio. Vendor pack-change rows still
 * appear in `rows` for UI rendering but don't distort the percentage.
 *
 * @param {import('better-sqlite3').Database} db
 * @param {string} locationId
 */
export function computeUnmapped(db, locationId) {
  const rows = /** @type {UnmappedBomRow[]} */ (
    db
      .prepare(
  ...
  const yieldKeyRows = /** @type {{ ingredient_key: string }[]} */ (
    db.prepare('SELECT ingredient_key FROM ingredient_yields').all()
  );
  const yieldKeys = new Set(yieldKeyRows.map((r) => r.ingredient_key));

  // `no_cost_utility` rows (tap water, etc.) are intentionally costless and
  // never contribute to BOM coverage — exclude them from the denominator and
  // skip all data-quality checks. They are NOT a coverage gap.
  const accountedRows = rows.filter((r) => r.map_status !== 'no_cost_utility');
  const totalItems = accountedRows.length;
  const unmapped = [];
  ...
  // test DB.
  const bomUnmappedCount = unmapped.length;

  /** @type {PackChangedRow[]} */
  let packChanged = [];
  try {
    packChanged = /** @type {PackChangedRow[]} */ (
      db.prepare(
  ...
  const unmappedPct = totalItems > 0
    ? Math.round((bomUnmappedCount / totalItems) * 10000) / 100
    : 0;

  return {
    total_items: totalItems,
    unmapped_count: bomUnmappedCount,
...
/**
 * @typedef {{ id: number, kind: string, started_at: string, finished_at: string | null, rows_in: number | null, rows_out: number | null, status: string | null }} IngestRunRow
 */
...
export function readLastCostingIngest(db) {
  const row = /** @type {IngestRunRow | undefined} */ (
    db
      .prepare(
