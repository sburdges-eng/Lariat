...
export function resolveMergedCost(rows, preferredVendor) {
  ...
  if (n === 0) return null;
  const meanUnitCost = latest.reduce((s, r) => s + effectivePackPrice(r) / r.pack_size, 0) / n;
  ...
  const canon = normalizeUnit(latest[0].pack_unit);
  const uniformUnit =
...
export function computeCostVariance(db, locationId, opts = {}) {
  ...
  for (const row of db.prepare(
    `SELECT ingredient_key, g_per_ml FROM ingredient_densities`,
  ).all()) {
