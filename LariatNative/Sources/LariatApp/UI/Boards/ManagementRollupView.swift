                ...
                ScrollView {
                    VStack(alignment: .leading, spacing: LaRiOS.Spacing.eight) {
                        LaRiOSBoardHeader(
                            eyebrow: "Manager",
                            title: "Management",
                            subtitle: "Live costing, price, depletion, and pack-size signals."
                        )
                        if let err = vm.errorText {
                            StaleDataBanner(message: err)
                        }
                        LazyVGrid(columns: [GridItem(.adaptive(minimum: 220))], spacing: LaRiOS.Spacing.eight) {
...
private struct Tile: View {
    ...
    var body: some View {
        LaRiOSMetricCard(title: title, value: value, caption: sub, tone: Self.tone(severity))
    }

    static func tone(_ severity: ThresholdColor?) -> LaRiOSTone {
        switch severity {
        case .red: return .bad
