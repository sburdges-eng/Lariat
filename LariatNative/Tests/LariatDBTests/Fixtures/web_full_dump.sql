-- Generated by scripts/dump-fresh-schema.mjs (mode: full).
...
CREATE TABLE ops_run_templates (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      location_id TEXT NOT NULL DEFAULT 'default',
...
CREATE TABLE ops_run_template_steps (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
...
CREATE TABLE toast_subscription_invoices (
...
CREATE TABLE toast_subscription_invoice_lines (
      id           INTEGER PRIMARY KEY AUTOINCREMENT,
      invoice_no   TEXT NOT NULL,
...
CREATE TABLE employee_health_acknowledgments (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      location_id TEXT DEFAULT 'default',
      cook_id TEXT NOT NULL,
...
CREATE TABLE entities_menu_items (
      uuid TEXT PRIMARY KEY,
      display_name TEXT NOT NULL,
      category TEXT,
      base_price REAL,
...
CREATE TABLE entities_ingredients (
      uuid TEXT PRIMARY KEY,
      display_name TEXT NOT NULL,
      ingredient_key TEXT NOT NULL,
      category TEXT,
...
CREATE TABLE entities_purchase_orders (
      uuid TEXT PRIMARY KEY,
...
CREATE TABLE sevenshifts_users (
...
CREATE TABLE sevenshifts_shifts (
      seven_id TEXT NOT NULL,
      location_id TEXT NOT NULL DEFAULT 'default',
      user_seven_id TEXT,
      employee_uuid TEXT,
...
CREATE INDEX idx_lce_shift ON line_check_entries(shift_date, station_id);
CREATE INDEX idx_signoff_shift ON station_signoffs(shift_date, station_id);
CREATE INDEX idx_86_shift ON eighty_six(shift_date, resolved_at);
CREATE INDEX idx_inv_shift ON inventory_updates(shift_date, station_id);
...
CREATE INDEX idx_inv_par_loc_cat
      ON inventory_par(location_id, category, ingredient);
CREATE INDEX idx_prep_tasks_loc_date
      ON prep_tasks(location_id, shift_date, status);
CREATE INDEX idx_prep_tasks_station
...
CREATE INDEX idx_vph_ingredient
      ON vendor_prices_history(ingredient);
CREATE INDEX idx_recipe_photos_slug
      ON recipe_photos(location_id, recipe_slug, id DESC)
      WHERE deleted_at IS NULL;
CREATE INDEX idx_margin_snapshots_loc_id
      ON margin_snapshots(location_id, id DESC);
CREATE INDEX idx_dish_coverage_snapshots_loc_id
      ON dish_coverage_snapshots(location_id, id DESC);
CREATE INDEX idx_accounting_variance_loc_id
      ON accounting_variance(location_id, id DESC);
CREATE INDEX idx_bom_recipe ON bom_lines(recipe_id, location_id);
CREATE INDEX idx_ingest_runs_kind_started ON ingest_runs(kind, started_at DESC);
CREATE INDEX idx_sales_loc ON sales_lines(location_id);
CREATE INDEX idx_dish_components_dish
...
CREATE INDEX idx_beo_prep_hist_loc_source
      ON beo_prep_history(location_id, source);
CREATE INDEX idx_equip_loc ON equipment(location_id);
CREATE INDEX idx_equip_maint_eq ON equipment_maintenance(equipment_id);
CREATE INDEX idx_equip_parts_eq ON equipment_parts(equipment_id);
CREATE INDEX idx_equip_sched_eq ON equipment_maintenance_schedule(equipment_id);
CREATE INDEX idx_temp_log_shift ON temp_log(shift_date, location_id, point_id);
CREATE INDEX idx_toast_daily_loc_date ON toast_sales_daily(location_id, shift_date);
CREATE INDEX idx_toast_sub_inv_date ON toast_subscription_invoices(location_id, invoice_date);
CREATE INDEX idx_toast_sub_lines_inv ON toast_subscription_invoice_lines(location_id, invoice_no);
CREATE INDEX idx_toast_sub_lines_item ON toast_subscription_invoice_lines(location_id, item, invoice_date);
CREATE INDEX idx_cbo_drain
      ON cloud_bridge_outbox(dead_letter, claimed_at, id);
CREATE INDEX idx_cooling_status
      ON cooling_log(location_id, status, shift_date);
CREATE INDEX idx_cooling_open
      ON cooling_log(location_id, started_at)
      WHERE status = 'in_progress';
CREATE INDEX idx_datemarks_active
      ON date_marks(location_id, discard_on)
      WHERE discarded_at IS NULL;
CREATE INDEX idx_receiving_shift
      ON receiving_log(location_id, shift_date);
CREATE INDEX idx_receiving_shellstock
...
CREATE UNIQUE INDEX idx_receiving_log_sync_source
...
CREATE UNIQUE INDEX idx_inventory_updates_sync_source
...
CREATE INDEX idx_lce_loc_date ON line_check_entries(location_id, shift_date);
CREATE INDEX idx_signoff_loc ON station_signoffs(location_id, shift_date);
CREATE INDEX idx_86_loc_date ON eighty_six(location_id, shift_date);
...
CREATE INDEX idx_vp_master ON vendor_prices(master_id);
CREATE INDEX idx_bom_master ON bom_lines(master_id);
CREATE INDEX idx_perf_review_cook ON performance_reviews(cook_name, cook_uuid, location_id);
CREATE INDEX idx_lari_conversation_partition
      ON lari_conversation_turns(location_id, cook_id, conversation_session_id, created_at DESC, id DESC);
CREATE INDEX idx_lari_conversation_expiry
      ON lari_conversation_turns(expires_at);
CREATE UNIQUE INDEX idx_service_hours_null_safe
        ON service_hours(location_id, day_of_week)
        WHERE service_label IS NULL;
CREATE UNIQUE INDEX idx_preshift_null_safe
        ON preshift_notes(location_id, shift_date)
        WHERE service_label IS NULL;
CREATE INDEX idx_shows_date ON shows(location_id, show_date);
CREATE INDEX idx_shows_band ON shows(location_id, band_name);
CREATE INDEX idx_shows_archive_date ON shows_archive(location_id, show_date);
CREATE INDEX idx_shows_archive_band ON shows_archive(location_id, band_name);
CREATE INDEX idx_stage_setups_show
      ON stage_setups(show_id, location_id);
CREATE INDEX idx_sound_scenes_show
      ON sound_scenes(show_id, location_id);
CREATE INDEX idx_spl_readings_show
      ON spl_readings(show_id, location_id, taken_at);
CREATE INDEX idx_box_office_show
      ON box_office_lines(show_id, location_id);
CREATE INDEX idx_box_office_source_ext
      ON box_office_lines(source, external_ref);
CREATE UNIQUE INDEX idx_box_office_external_ref_unique
      ON box_office_lines(source, external_ref)
      WHERE external_ref IS NOT NULL;
CREATE INDEX idx_show_deals_show
      ON show_deals(show_id, location_id);
CREATE INDEX idx_specials_loc_created
      ON specials(location_id, created_at DESC);
CREATE INDEX idx_specials_active
...
CREATE INDEX idx_specials_promotions_menu_item
      ON specials_promotions(location_id, menu_item_name);
CREATE INDEX idx_kds_tickets_active
...
INSERT INTO "locations" ("id", "name", "created_at", "capacity", "tax_rate", "service_fee_pct", "phone", "address") VALUES ('default', 'The Lariat', '2026-08-29 05:41:22', NULL, 0.0675, 20, NULL, NULL);
INSERT INTO "schema_migrations" ("version", "applied_at") VALUES (6, '1970-01-01 00:00:00');
