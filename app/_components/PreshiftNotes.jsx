...
/** @typedef {import('../../lib/db').PreshiftNote} PreshiftNote */
...
export default function PreshiftNotes({ initialNote, shiftDate, serviceLabel, locationId }) {
  const [note, setNote] = useState(/** @type {PreshiftNote | null} */ (initialNote || null));
  ...
  const save = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        const data = /** @type {{ note: PreshiftNote }} */ (await res.json());
        setNote(data.note);
        setEditing(false);
      }
    } catch {
      ...
      {editing ? (
        <>
          <textarea
            ...
            rows={5}
            value={draft}
            onChange={/** @param {React.ChangeEvent<HTMLTextAreaElement>} e */ (e) => setDraft(e.target.value)}
        ...
        // Reaching this branch means editing is false, and the guard above
        // (`!note && !editing`) already returned early otherwise — so note
        // is guaranteed non-null here. The `note &&` re-check is only to
        // satisfy TS's control-flow analysis (it can't derive that from the
        // compound `!note && !editing` condition above); it is never false
        // at runtime.
        note && (
          <>
