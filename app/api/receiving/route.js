...
async function receivingHandler(req) {
  try {
    ...
    const shouldCreditInventory =
      shouldAttemptInventoryCredit &&
      masterMatch.status === 'matched' &&
      typeof masterMatch.master_id === 'string' &&
      masterMatch.master_id.length > 0;
    ...
    const performWrite = db.transaction(() => {
      ...
      appendOp({
        ...
        rowJson: JSON.stringify({
          ...
          item,
          vendor_sku,
          master_id: masterMatch.master_id,
          match_status: masterMatch.status,
          match_reason: masterMatch.reason,
          reading_f,
